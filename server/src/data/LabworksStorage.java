...
public class LabworksStorage implements Serializable {
    ...
    public static String put(LabWork l){
        ...
        if (added){ return "Labwork " + l.getName() + " добавлена. Id = " + l.getId(); }
        else { return "Лабораторная работа уже добавлена."; }
    }

    public static void setData(PriorityQueue<LabWork> labs){
        labworks = labs;
    }
    ...
    public static String remove(LabWork lab){
        labworks.remove(lab);
        String res = "Labwork с ID: " + lab.getId() + " удален.";
        return res;
    }

    public static LabWork searchById(int id){
        LabWork laboratory =  null;
        Optional<LabWork> lab = labworks.stream().filter(l -> id == l.getId()).findAny();
        try{
            laboratory = lab.get();
